...
createGlobalPropertyf("an-24rv/set/sound_volume", 1000) -- global sound volume
...
defineProperty("set_real_ahz", globalPropertyi("an-24rv/set/real_ahz")) -- real ahz has errors and needs to be corrected
...
defineProperty("set_active_camera", globalPropertyi("an-24rv/set/active_camera")) -- use of moveing camera
defineProperty("set_real_generators", globalPropertyi("an-24rv/set/real_generators")) -- generators can fail if overload
defineProperty("set_real_gears", globalPropertyi("an-24rv/set/real_gears")) --  gears can fail in overspeed
defineProperty("set_real_brakes", globalPropertyi("an-24rv/set/real_brakes")) -- brakes can overheat and fail
defineProperty("set_real_tyres", globalPropertyi("an-24rv/set/real_tyres")) -- tyres can blow if too much brakes
defineProperty("hide_GPS", globalPropertyi("an-24rv/set/hide_GPS")) -- by default GPS is hided
defineProperty("hide_Garmin", globalPropertyi("an-24rv/set/hide_Garmin")) -- by default Garmin is hided
defineProperty("switch_rud", globalPropertyi("an-24rv/set/switch_rud")) -- switch or hold rud stopors
defineProperty("rud_close", globalPropertyi("an-24rv/misc/rud_close")) -- down RUD latch when hold mode is ON
defineProperty("north_GPK", globalPropertyi("an-24rv/set/north_GPK"))  -- GPK mode for north hemisphere
...
function file_read()
	local filename = panelDir .. "/an-24rv_settings.ini"
	...
	if file then
		...
		set(kln_l_is_present, settings_table["klnlispresent"])
		set(kln_r_is_present, settings_table["klnrispresent"])
...
function file_save()
	...
	savefile:write("klnlispresent","=",get(kln_l_is_present)," \n")
	savefile:write("klnrispresent","=",get(kln_r_is_present)," \n")
  ...
  switchLit {
    ...
    onMouseClick = function()
      if not switcher_pushed then
        playSample(switch_sound, 0)
        switcher_pushed = true
        if get(kln_l_is_present) == 0 then
          set(kln_l_is_present, 1)
  ...
  switchLit {
    ...
    onMouseClick = function()
      if not switcher_pushed then
        playSample(switch_sound, 0)
        switcher_pushed = true
        if get(kln_r_is_present) == 0 then
          set(kln_r_is_present, 1)
          -- if get(hide_Garmin) == 0 then set(hide_Garmin, 1) end
        else
          set(kln_r_is_present, 0)
        end
      end
      return true
    end,
    ...
    onMouseClick = function()
      if not switcher_pushed then
        ...
        if get(hide_Garmin) ~= 0 then
          set(hide_Garmin, 0)
          if get(kln_l_is_present) == 1 then set(kln_l_is_present, 0) end
        else
          set(hide_Garmin, 1)
        end
      end
