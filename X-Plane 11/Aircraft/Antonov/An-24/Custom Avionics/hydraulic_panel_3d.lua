...
-- define properties
defineProperty("main_press", globalPropertyf("an-24/hydro/main_press")) -- pressure in main system. initial 120 kg per square sm. maximum 160.
...
defineProperty("hydro_quantity", globalPropertyf("an-24/hydro/hydro_quantity")) -- quantity of hydraulic liquid. initially 28 liters. in work downs to 21 liters. also can flow out in come case of failure.
defineProperty("hydro_store", globalPropertyf("an-24/hydro/hydro_store")) -- pressure in main hydro storage
...
defineProperty("gear_valve", globalPropertyi("an-24/hydro/gear_valve")) -- position of gear valve for gydraulic calculations and animations.
defineProperty("gear_rotary", globalPropertyi("an-24/hydro/gear_rotary")) -- position of gear valve for gydraulic calculations and animations.
defineProperty("gear_unblock", globalPropertyi("an-24/hydro/gear_unblock")) -- remove block from gear retraction on ground
defineProperty("gear_unblock_cap", globalPropertyi("an-24/hydro/gear_unblock_cap")) -- remove block from gear retraction on ground
...
defineProperty("flaps_valve_emerg_cap", globalPropertyi("an-24/hydro/flaps_valve_emerg_cap")) -- position of emergency flaps valve for gydraulic calculations and animations.
defineProperty("emerg_pump_sw", globalPropertyi("an-24/hydro/emerg_pump_sw")) -- emergency hydro pump switcher. if its ON and power exist - emergency bus will gain pressure
...
registerCommandHandler(createCommand("An-24/Instruments/emerg_pump_sw_on", "Emergency hydro pump switch on."), 0, function(p) if p == 0 and get(emerg_pump_sw) ~= 1 then set(emerg_pump_sw, 1) end return 0 end)
registerCommandHandler(createCommand("An-24/Instruments/emerg_pump_sw_off", "Emergency hydro pump switch off."), 0, function(p) if p == 0 and get(emerg_pump_sw) ~= 0 then set(emerg_pump_sw, 0) end return 0 end)
registerCommandHandler(createCommand("An-24/Instruments/emerg_pump_sw_toggle", "Emergency hydro pump switch toggle."), 0, function(p) if p == 0 then if get(emerg_pump_sw) == 0 then set(emerg_pump_sw, 1) else set(emerg_pump_sw, 0) end end return 0 end)
...
function update()
	...
	if get(block_brake) > 0 then
		brake_1_angle = (-math.max(get(brake1), get(block_brake)) * power115 * 120 / 1.5) + (-math.min(get(brake1), get(block_brake)) * power115 * 20) + 195
	else
		brake_1_angle = -math.max(get(brake1), get(block_brake)) * power115 * 120 / 1.5 + 195
	end
	...
	if get(block_brake) > 0 then
		brake_2_angle = (math.max(get(brake2), get(block_brake)) * power115 * 120 / 1.5) + (math.min(get(brake2), get(block_brake)) * power115 * 20) - 105
	else
		brake_2_angle = math.max(get(brake2), get(block_brake)) * power115 * 120 / 1.5 - 105
	end
