...
defineProperty("nav_light", globalPropertyf("an-24/misc/nav_light")) -- nav light can shine
defineProperty("beacon_light", globalPropertyf("an-24/misc/beacon_light")) -- beacon light ratio
defineProperty("azs_beacon_down", globalPropertyf("an-24/beacon_down"))
defineProperty("azs_beacon_up", globalPropertyf("an-24/beacon_up"))
...
defineProperty("cockpit_light_cc", globalPropertyf("an-24/misc/cockpit_light_cc")) -- light current consumption
...
local headlight = globalPropertyf("an-24/anim/headlight")
...
registerCommandHandler(createCommand("An-24/Lights/inst_lights_on", "Instruments lights on (main)."), 0, function(p) if p == 0 and get(cockpit_panel) ~= 1 then set(cockpit_panel, 1) end return 0 end)
registerCommandHandler(createCommand("An-24/Lights/inst_lights_emerg", "Instruments lights on (emergency)."), 0, function(p) if p == 0 and get(cockpit_panel) ~= -1 then set(cockpit_panel, -1) end return 0 end)
registerCommandHandler(createCommand("An-24/Lights/inst_lights_off", "Instruments lights off."), 0, function(p) if p == 0 and get(cockpit_panel) ~= 0 then set(cockpit_panel, 0) end return 0 end)
registerCommandHandler(createCommand("An-24/Lights/inst_lights_up", "Instruments lights switch up."), 0, function(p) if p == 0 and get(cockpit_panel) ~= 1 then set(cockpit_panel, get(cockpit_panel)+1) end return 0 end)
registerCommandHandler(createCommand("An-24/Lights/inst_lights_down", "Instruments lights switch down."), 0, function(p) if p == 0 and get(cockpit_panel) ~= -1 then set(cockpit_panel, get(cockpit_panel)-1) end return 0 end)
registerCommandHandler(createCommand("An-24/Lights/tl_lights_land", "Landing lights on."), 0, function(p) if p == 0 and get(lan_light_sw) ~= 1 then set(lan_light_sw, 1) end return 0 end)
registerCommandHandler(createCommand("An-24/Lights/tl_lights_taxi", "Taxi lights on."), 0, function(p) if p == 0 and get(lan_light_sw) ~= -1 then set(lan_light_sw, -1) end return 0 end)
registerCommandHandler(createCommand("An-24/Lights/tl_lights_off", "Taxi/Landing lights off."), 0, function(p) if p == 0 and get(lan_light_sw) ~= 0 then set(lan_light_sw, 0) end return 0 end)
registerCommandHandler(createCommand("An-24/Lights/tl_lights_up", "Taxi/Landing lights switch up."), 0, function(p) if p == 0 and get(lan_light_sw) ~= 1 then set(lan_light_sw, get(lan_light_sw)+1) end return 0 end)
registerCommandHandler(createCommand("An-24/Lights/tl_lights_down", "Taxi/Landing lights switch down."), 0, function(p) if p == 0 and get(lan_light_sw) ~= -1 then set(lan_light_sw, get(lan_light_sw)-1) end return 0 end)

registerCommandHandler(createCommand("An-24/Lights/tl_lights_ext", "Taxi/Landing lamp extend."), 0,
function(p)
  if get(real_hl) == 0 then
    if p == 0 and get(lan_light_open_sw) ~= 1 then set(lan_light_open_sw, 1) end
  else
    if p == 1 then set(lan_light_open_sw, 1) else set(lan_light_open_sw, 0) end
  end
  return 0
end)

registerCommandHandler(createCommand("An-24/Lights/tl_lights_ret", "Taxi/Landing lamp retract."), 0,
function(p)
  if get(real_hl) == 0 then
    if p == 0 and get(lan_light_open_sw) ~= -1 then set(lan_light_open_sw, -1) end
  else
...
registerCommandHandler(createCommand("An-24/Lights/tl_lights_toggle", "Taxi/Landing lamp toggle."), 0,
function(p)
  if get(real_hl) == 0 then
    if p == 0 then
      if get(lan_light_open_sw) == -1 then
        set(lan_light_open_sw, 1)
      else
        set(lan_light_open_sw, -1)
      end
...
function smooth(value)
  local spd = 0.01
  if value == 1 and lan_light_angle < 1 then
    lan_light_angle = lan_light_angle + spd
  elseif value == -1 and lan_light_angle > 0 then
    lan_light_angle = lan_light_angle - spd
  end
  return lan_light_angle
end

function check_switch()
  if (get(real_hl) == 0 and get(lan_light_open_sw) == 0) then set(lan_light_open_sw,-1) end
  if (get(real_hl) == 1 and (get(lan_light_open_sw) == -1 or get(lan_light_open_sw) == 1)) then set(lan_light_open_sw,0) end
  last_hl_set = get(real_hl)
end
...
function update()
  ...
  if get(real_hl) ~= last_hl_set then check_switch() end
  ...
  local ll_open_sw = get(lan_light_open_sw)
  -- nav light logic
  local nav_bright = 0
  ...
  -- open landing light
  if power27 > 21 and ll_open_sw ~= 0 then set(headlight, smooth(ll_open_sw)) end
